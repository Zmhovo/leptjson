...
static void* lept_context_push(lept_context* c, size_t size) {
	...
	if (c->top + size >= c->size)
	{
		...
		{
			c->size += c->size >> 1;     /*循环以1.5倍扩展堆栈大小直到可以容纳*/
		}
		c->stack = (char*)realloc(c->stack, c->size);
	}
	ret = c->stack + c->top;     /*返回当前栈顶指针位置*/
	c->top += size;
	return ret;
}
...
static int lept_parse_number(lept_context* c, lept_value* v){
	const char* p = c->json;     /*记录当前字符串的位置*/
	...
	v->u.n = strtod(c->json, NULL);     /*strtod()将十进制转换为二进制double*/
	if (errno == ERANGE && (v->u.n == HUGE_VAL || v->u.n == -HUGE_VAL))     /*溢出检查*/
	{
                ...
                switch (*p++) {
					...
					case 'u':
						...
						if (u >= 0xD800 && u <= 0xDBFF)
						{
							...
							u = (((u - 0xD800) << 10) | (u2 - 0xDC00)) + 0x10000;     /*代理对处理*/
						}
...
static void lept_stringify_string(lept_context* c, const char* s, size_t len)
{
    ...
    char* head, *p;
    assert(s != NULL);
	p = head = lept_context_push(c, size = len * 6 + 2);     /*预分配内存。len * 6是处理可能存在的转义字符，每个字符最多转义为6个字符。+2为添加开头结尾的"*/
	*p++ = '"';
	for (i = 0; i < len; i++)
	{
...
void lept_copy(lept_value* dst, const lept_value* src)     /*将src深拷贝到dst*/
{
...
void lept_set_string(lept_value* v, const char* s, size_t len) {
	...
	memcpy(v->u.s.s, s, len);     /*将长度为len的字符串s复制到v->u.s.s*/
	v->u.s.s[len] = '\0';
	v->u.s.len = len;
	v->type = LEPT_STRING;
}
...
void lept_reserve_array(lept_value* v, size_t capacity)     /*扩充数组容量*/
{
	assert(v != NULL && v->type == LEPT_ARRAY);
...
lept_value* lept_pushback_array_element(lept_value* v)     /*尾插元素*/
{
	assert(v != NULL && v->type == LEPT_ARRAY);
...
void lept_popback_array_element(lept_value* v)     /*弹出尾部元素*/
{
